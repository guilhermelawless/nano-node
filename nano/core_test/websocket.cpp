...
TEST (websocket, confirmation_options_update)
{
	...
	auto task = ([&updated, config, &node1]() {
		fake_websocket_client client (config.websocket_config.port);
		...
		std::string update_message = boost::str (boost::format (R"json({"action": "update", "topic": "confirmation", "ack": "true", "options": {"accounts_add": ["%1%"]}})json") % nano::test_genesis_key.pub.to_account ());
