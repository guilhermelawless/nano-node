...
keepalive_timer{ node_a.io_ctx }
{
	...
	for (size_t i = 0; i < node.config.network_threads && !node.flags.inactive_node; ++i)
	{
...
{
	auto result (tcp_channels.random_set (count_a, min_version_a, include_temporary_channels_a));
	debug_assert (result.size () <= count_a);
	return result;
}
...
std::optional<nano::tcp_endpoint> nano::network::bootstrap_peer ()
{
	auto peer = tcp_channels.bootstrap_peer (node.network_params.protocol.protocol_version_min (node.ledger.cache.epoch_2_started));
	if (peer == nano::tcp_endpoint (boost::asio::ip::address_v6::any (), 0))
	{
		return {};
	}
	return peer;
