...
void nano::vote_processor::process_loop ()
{
	...
	while (!stopped)
	{
		if (!votes.empty ())
		{
			...
			{
				std::cout << (boost::str (boost::format ("Processed %1% votes in %2% milliseconds (rate of %3% votes per second)") % votes_l.size () % elapsed.value ().count () % ((votes_l.size () * 1000ULL) / elapsed.value ().count ()))) << std::endl;
			}
		}
		else
		{
