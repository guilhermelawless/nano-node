...
{
public:
	rpc_session (boost::property_tree::ptree const & request, boost::asio::io_context & ioc, tcp::resolver::results_type const & results, std::function<void(boost::system::error_code const &, boost::optional<boost::property_tree::ptree> const &)> callback) :
	io_ctx (ioc),
	socket (ioc),
	request (request),
	results (results),
		...
		boost::asio::async_connect (this_l->socket, this_l->results.cbegin (), this_l->results.cend (), [this_l](boost::system::error_code const & ec, boost::asio::ip::tcp::resolver::iterator) {
			...
			http::async_write (this_l->socket, this_l->req, [this_l](boost::system::error_code ec, std::size_t) {
				...
				http::async_read (this_l->socket, this_l->buffer, this_l->res, [this_l](boost::system::error_code ec, std::size_t) {
					...
					return this_l->callback (ec, json);
				});
			});
		});
	}
...
private:
	...
	boost::property_tree::ptree request;
	tcp::resolver::results_type const & results;
	std::function<void(boost::system::error_code const &, boost::optional<boost::property_tree::ptree> const &)> callback;
};
...
boost::property_tree::ptree rpc_request (boost::property_tree::ptree const & request, boost::asio::io_context & ioc, tcp::resolver::results_type const & results)
{
	...
	auto rpc_session (std::make_shared<rpc_session> (request, ioc, results, [&promise](boost::system::error_code const & ec, boost::optional<boost::property_tree::ptree> const & response) {
	...
	while (future.wait_for (std::chrono::seconds (0)) != std::future_status::ready)
	{
		...
		{
			std::cerr << "Timeout while performing an RPC request" << std::endl;
			release_assert (false);
		}
		std::this_thread::sleep_for (std::chrono::milliseconds (50));
	}
	auto response = future.get ();
	release_assert (response.is_initialized ());
	return *response;
}
