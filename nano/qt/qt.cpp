			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						if (!wallet.node.work_generate_blocking (send))
						{
							std::string block_l;
						...
						{
							show_label_error (*status);
							status->setText ("Work generation failed");
						}
					}
					else
					{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					...
					if (!wallet.node.work_generate_blocking (change))
					{
						std::string block_l;
					...
					if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								...
								if (!wallet.node.work_generate_blocking (open))
								{
									std::string block_l;
